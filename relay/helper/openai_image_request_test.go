...
func TestImageTierRatioKeepsQuantitySeparate(t *testing.T) {
	for _, tc := range []struct {
		model string
		want  float64
	}{
		{model: "tier-image", want: 1.5},
		{model: "dall-e-3", want: 3},
	} {
		t.Run(tc.model, func(t *testing.T) {
			request := dto.ImageRequest{Model: tc.model, Size: "1024x1792", Quality: "hd", N: common.GetPointer(uint(3)), TierSizeRatio: common.GetPointer(1.5)}
			meta := request.GetTokenCountMeta()
			require.NotNil(t, meta)
			assert.Equal(t, tc.want, meta.ImagePriceRatio)
			assert.Equal(t, 3.0, meta.BillingRatios["n"])
			encoded, err := common.Marshal(request)
			require.NoError(t, err)
			assert.NotContains(t, string(encoded), "TierSizeRatio")
		})
	}
}
